...
class DBRecord:
    ...
    def __init__(self, tableID, dataBuff, mapBuff, recID=""):
        ...
        self.recIDBuff = create_string_buffer(
            bytes(recID, "ascii"), self.recIDBuffSize)
...
class DataRequest:
    ...
    def _parse_query(self, query):
        ...
        _where = Suppress(Literal('WHERE')) + ZeroOrMore(lparen) + OneOrMore(
            Group(_conditional) + ZeroOrMore(joiner)) + ZeroOrMore(rparen)
...
class FieldAttributesRequest:
    ...
    def _query_to_dict(self, query):
        ...
        _select = Suppress('SELECT') + Suppress('(') + \
            delimitedList(Word(alphas)) + Suppress(')')
...
class DBInterface:
    ...
    def __init__(self, appName='CommitAgent', CRMPath=r'C:\CommitCRM'):
        ...
        self.CmDBEngDll = windll.LoadLibrary(
            self.serverPath + r'\cmtdbeng.dll')
        self.CmDBQryDll = windll.LoadLibrary(
            self.serverPath + r'\cmtdbqry.dll')
    ...
    def _init_db_eng_dll(self):
        self.CmDBEngDll.CmtInitDbEngDll(
            self.appName, self.DBPath_bytes, byref(
                self.status))
    ...
    def _init_db_qry_dll(self):
        self.CmDBQryDll.CmtInitDbQryDll(
            self.appName, self.DBPath_bytes, byref(
                self.status))
